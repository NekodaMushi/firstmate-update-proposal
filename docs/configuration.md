...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and per-task gate files with their results (see "Task gates" below).
...
The checker resolves the task's isolated copy from `worktree=` in `state/<id>.meta`, and an ordinary gauge run checks that copy as it stands on disk, uncommitted changes included, while the decisive run firstmate uses for acceptance holds the copy to a delivered commit instead.
...
FM_GATES_TIMEOUT=120   # seconds allowed per gates.md CHECK in bin/fm-gates-check.sh; --timeout overrides it (see "Task gates")
