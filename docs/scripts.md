...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, Herdr-lab, and acceptance-gate briefs |
...
| `fm-relaunch.sh`         | Replace one ship or scout task's agent in its own recorded endpoint and worktree, refusing unless its prewritten relaunch note exists and no live validation run owns the branch |
