...
`bin/fm-promote.sh` flips the contract without touching gates, so first archive the scout's `gates.md` and `gates-result.md` together and unedited under `data/<id>/archive/scout/`, deleting neither, then immediately write the fresh ship `gates.md`; promotion is incomplete and the promoted task is not dispatched until that file exists.
